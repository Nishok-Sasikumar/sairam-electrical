...
// Firebase Config (Using Client SDK as Service Account is not available locally)
...
app.post("/api/orders/place", async (req, res) => {
  try {
    console.log(`[ORDER] Received order placement request for user: ${req.body.userId}`);
    const { 
      userId, 
      userName, 
      userEmail, 
    ...
    console.log(`[ORDER] Order created with ID: ${docRef.id}`);
...
// Add a GET test route
app.get("/api/orders/place", (req, res) => {
  res.json({ message: "Order placement endpoint is ready for POST requests." })
})


// Request Logger (Debug 404)

// Temporary In-Memory OTP Store (Fixes Firestore Permission Issues)
...
const sendEmail = async ({ to, subject, htmlContent }) => {
  try {
    ...
    // If API key is wrong, tell the user exactly what's wrong
    if (err.response?.status === 401) {
      throw new Error("Invalid Brevo API Key. Please ensure your EMAIL_PASS in .env is a valid API Key (starting with xkeysib-). Your current key looks like an SMTP password.");
    }
...
app.post("/api/enquiry/submit", async (req, res) => {
  try {
    const { name, email, phone, message, userId, firestoreId } = req.body
    
    let firestoreSaved = !!firestoreId;
    
    // Only try to save if frontend hasn't already saved it
    if (!firestoreSaved) {
      try {
        ...
        console.error("Firestore Save Error (Backend blocked by rules):", fsErr.message)
      }
    }
    ...
    const htmlContent = `
        ...
        ${!firestoreSaved ? '<p style="color: red; font-size: 10px;">Warning: This enquiry was not saved to Firestore database. It only exists in this email.</p>' : ''}
      `;
    ...
    res.json({ 
      success: true, 
      message: firestoreSaved ? "Enquiry submitted and admin notified" : "Enquiry sent via email (Database sync failed)",
      firestoreStatus: firestoreSaved ? "saved" : "skipped"
    })
  } catch (err) {
    console.error("Enquiry submission fatal error:", err)
    res.status(500).json({ success: false, error: "Submission failed" })
  }
})

// OTP: Send (Uses In-Memory Store to bypass Firestore Permission Denied)
app.post("/api/otp/send", async (req, res) => {
  ...
  try {
    ...
    otpStore.set(email, { code, expires: expiration });
    console.log(`OTP ${code} stored in memory for ${email}`);
